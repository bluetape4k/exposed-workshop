...
class Ex10_DDL_Examples : AbstractExposedTest() {
    ...
    fun `keyword identifiers without opt out`(testDB: TestDB) {
        ...
        withDb(testDB) {
            ...
            val (tableName, publicName, dataName, constraintName) =
                keywords.map {
    ...
    fun `binary columns`(testDB: TestDB) {
        ...
        withTables(testDB, t) {
            ...
            val worldByBitCol =
                t
                    .selectAll()
                    .where { t.byteCol eq byteArrayOf(1) }
                    .readAsString()
                    .single()
    ...
    fun `table with different text type`(testDB: TestDB) {
         ...
         * Postgres:
         * ```sql
         * CREATE TABLE IF NOT EXISTS different_text_column_types (
         *      id SERIAL PRIMARY KEY,
         *      txt TEXT NOT NULL,
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `check constraints 01`(testDB: TestDB) {
        Assumptions.assumeTrue { testDB != TestDB.MYSQL_V5 }
