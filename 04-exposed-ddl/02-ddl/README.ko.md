...
![Architecture diagram](../../docs/images/readme-diagrams/04-exposed-ddl-02-ddl-architecture-01.png)

## 테이블 클래스 계층

![Table diagram](../../docs/images/readme-diagrams/04-exposed-ddl-02-ddl-class-02.png)

## 복합 PK / FK 관계 ERD

![PK / FK Relation ERD diagram](../../docs/images/readme-diagrams/04-exposed-ddl-02-ddl-erd-03.png)
...
transaction {
    ...
    // 누락된 테이블/컬럼에 대한 마이그레이션 구문을 생성한 뒤 각각 실행
    val statements = MigrationUtils.statementsRequiredForDatabaseMigration(CityTable, UserTable)
    statements.forEach { sql -> exec(sql) }
    commit()
...
`MigrationUtils.statementsRequiredForDatabaseMigration`으로 마이그레이션에 필요한 DDL 구문을 생성한 뒤 각각 실행합니다. 이미 존재하는 테이블에 누락된
`uniqueIndex`를 추가하거나,
...
- `MigrationUtils.statementsRequiredForDatabaseMigration` 사용 시 예상치 못한 변경을 검토한다.
