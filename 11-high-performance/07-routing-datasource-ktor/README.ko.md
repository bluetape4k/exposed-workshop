...
이 모듈은 Ktor request handling과 Exposed JDBC로 명시적인 read/write datasource routing을 구현하는 예제입니다. `RoutingDataSourcePlugin`은 `GET` 요청을 `READ`, 변경 요청을 `WRITE`로 선택하고, 테스트와 진단을 위해 `X-Data-Source` header로 대표 요청을 override할 수 있습니다.
...
Spring transaction routing 없이 datasource selection이 필요한 Ktor 서비스 예제로 사용합니다.
