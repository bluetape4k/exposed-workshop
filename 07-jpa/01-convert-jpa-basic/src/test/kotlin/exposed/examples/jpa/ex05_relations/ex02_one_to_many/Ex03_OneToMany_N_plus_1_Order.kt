...
import exposed.examples.jpa.ex05_relations.ex02_one_to_many.schema.OrderSchema.Order
import exposed.examples.jpa.ex05_relations.ex02_one_to_many.schema.OrderSchema.OrderItem
...
class Ex03_OneToMany_N_plus_1_Order: AbstractExposedTest() {
    ...
    fun `one-to-many bidirectional`(testDB: TestDB) {
        withTables(testDB, *ordersTables) {
             ...
             * ```
             */
            val query: Query = OrderTable.innerJoin(OrderItemTable).selectAll().where { OrderTable.id eq order1.id }
