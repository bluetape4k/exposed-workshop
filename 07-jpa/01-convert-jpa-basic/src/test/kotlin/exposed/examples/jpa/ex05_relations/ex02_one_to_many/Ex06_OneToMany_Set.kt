...
class Ex06_OneToMany_Set: JdbcExposedTestBase() {
    ...
    object BidTable: IntIdTable("bids") {
        ...
        val itemId = reference("item_id", BiddingItemTable, onDelete = ReferenceOption.CASCADE)
    ...
    fun `중복 입찰을 허용하지 않아야 합니다`(testDB: TestDB) {
        withTables(testDB, BiddingItemTable, BidTable) {
            ...
            log.debug { "bid1=$bid1, bid2=$bid2, bid3=$bid3" }
    ...
    object ProductImageTable: IntIdTable("product_images") {
        ...
        val productId: Column<EntityID<Int>?> =
            optReference("product_id", ProductTable, onDelete = ReferenceOption.CASCADE)
