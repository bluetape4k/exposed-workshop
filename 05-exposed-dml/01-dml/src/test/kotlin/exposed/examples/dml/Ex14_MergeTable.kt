...
class Ex14_MergeTable: Ex14_MergeBase() {
    ...
    /**
     * whenMatchedUpdate를 사용하는 MergeFrom 예제
     *
     * ```sql
     * -- Postgres
     * MERGE INTO dest dest_alias
    ...
    /**
     * `whenMatchedDelete`를 사용하는 [mergeFrom] 예제 and condition
     *
    ...
    /**
     * 자동 생성된 ON condition을 사용하는 MergeFrom 예제
     *
     * Postgres:
     * ```sql
     * MERGE INTO dest
    ...
    /**
     * `whenNotMatchedDoNothing`을 사용하는 [mergeFrom] 예제
     *
    ...
    fun `with overridingSystemValue option in postgres`(testDB: TestDB) {
        ...
        withTables(testDB, source, dest) {
            ...
            dest.mergeFrom(source) {
                // `overridingSystemValue`는 `generated always as identity` 값을 덮어쓸 수 있게 합니다.
                // 그렇지 않으면 Postgres는 auto generated value를 덮어쓰면 안 된다는 예외를 던집니다.
    ...
    fun `with overridingUserValue option in postgres`(testDB: TestDB) {
        ...
        withTables(testDB, source, dest) {
            try {
                ...
                dest.mergeFrom(source) {
                    // 여기서 `overridingUserValue`는 source table의 id value 설정을 피하고 sequence가 생성한 값을 대신 사용하게 합니다.
    ...
    fun `insert default value`(testDB: TestDB) {
        ...
        val dest = object: IntIdTable("dest") {
            // 여기서 `withDefinition()`은 insert가 default value를 명시적으로 넘기지 않도록 하는 workaround입니다.
            val value = varchar("value", 128)
