...
DSL 체이닝으로 `innerJoin` + `groupBy` + 집계 함수를 조합합니다.
...
관련 테스트:
- [`ExposedSQLExample.kt` - `use functions and group by`](src/test/kotlin/exposed/sql/example/ExposedSQLExample.kt)
- [`ExposedSQLSuspendedExample.kt` - `use functions and group by`](src/test/kotlin/exposed/sql/example/ExposedSQLSuspendedExample.kt)

### andWhere / orWhere 체이닝

`.where()` 이후 `.andWhere()`, `.orWhere()`로 조건을 동적으로 추가합니다.
...
관련 테스트:
- [`ExposedSQLExample.kt` - `manual inner join`](src/test/kotlin/exposed/sql/example/ExposedSQLExample.kt)
