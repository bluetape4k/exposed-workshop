...
| 입력 단위 변환과 기준 단위 조회 | `Ex01MeasuredColumns.kt` (`cm`, `m`, `km`, `g`, `kg`, `°F`, `°C`, `K`) | `:13-exposed-measured:test` 12 tests, failures/errors 0 | PASS |
...
| 호환되지 않는 DB 값 거부와 계열 경계 | provider `valueFromDB` source contract와 README의 `Measure<Mass>` → `Measure<Length>` 컴파일 오류 예 | provider source ledger와 source-equivalent 문서 확인; 인위적인 driver 출력 주입 없음 | PASS |
...
| 4. JDBC round-trip/nullable/precision tests와 provider 실패 경계 문서화 | 완료 |
...
- `./gradlew :13-exposed-measured:test --no-build-cache --no-daemon --no-configuration-cache` — BUILD SUCCESSFUL; JUnit XML `tests=12`, `failures=0`, `errors=0`, `skipped=0`.
- `./gradlew :13-exposed-measured:test -PuseFastDB=true --no-build-cache --no-daemon --no-configuration-cache` — BUILD SUCCESSFUL; fast H2 JUnit XML `tests=4`, `failures=0`, `errors=0`, `skipped=0`.
