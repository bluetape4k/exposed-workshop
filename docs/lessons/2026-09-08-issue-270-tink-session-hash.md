...
- `bluetape4k-tink`와 직접 호환성을 고정해야 하는 `bluetape4k-core`를 배포된
  `2.1.0-20260907.141940-7`로 두 auth-session 모듈에 직접 pin한다. 안정 BOM
  `2.0.0`은 logging·virtualthread 같은 ancillary 전이에 적용되는 경계로
  유지하며, pinned Tink/core 계약을 `2.0.0`으로 낮추지 않는지 resolved graph에서
  확인한다.
...
- provider의 timestamped build를 직접 연결하면 BOM의 안정 버전 규칙이 전이 dependency를
  안정 버전으로 선택할 수 있다. Tink만 graph에 보이는지 확인하지 말고 provider가
  요구하는 core 버전도 `dependencyInsight`로 확인해 모듈 범위에 pin한다. logging과
  virtualthread ancillary 전이는 stable BOM 경계를 따르는지 별도로 기록한다.
...
- 두 모듈의 `runtimeClasspath`와 `dependencyInsight`에서 Tink/core가
  `2.1.0-20260907.141940-7`로 선택됐다. `logging`, `virtualthread-api`,
  `virtualthread-jdk25`는 안정 BOM의 `2.0.0` 경계를 따르는 ancillary 전이로
  확인했으며, pinned Tink/core에는 `2.1.0-SNAPSHOT -> 2.0.0` downgrade가 없었다.
...
중앙 catalog가 Tink/core 호환 계약을 안정 버전으로 발행하면 local timestamped
aliases와 직접 core pin을 제거하고 중앙 alias로 되돌린다. 그 전에는 consumer의
resolved graph에서 직접 pin한 Tink/core와 ancillary stable BOM 경계를 각각 확인한다.
